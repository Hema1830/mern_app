...
exports.postTask = async (req, res) => {
  try {
    const { description, dueDate, reminder, priority, status } = req.body;
    if (!description) {
      return res.status(400).json({ status: false, msg: "Description of task not found" });
    }
    const task = await Task.create({ user: req.user.id, description, dueDate, reminder, priority, status });
...
exports.putTask = async (req, res) => {
  try {
    ...
    task = await Task.findByIdAndUpdate(req.params.taskId, { description, dueDate, reminder, priority, status }, { new: true });
